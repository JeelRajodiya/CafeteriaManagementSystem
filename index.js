...
app.get("/", async (req, res) => {
  try {
    ...
    res.render("homepage", {
      ...
      error: "Sorry Some Error has occured",
    });
  } catch (err) {
    console.log(err);
  }
...
app.post("/caterer/product/new", async (req, res) => {
  try {
    ...
    const newProduct = await pool.query("CALL newProduct($1, $2, $3, $4,$5);", [
...
app.post("/caterer/product/delete", async (req, res) => {
  try {
    ...
    const queryResult = await pool.query("call deleteProduct($1);", [
...
app.get("/analytics", async (req, res) => {
  try {
    ...
    const maxPayingCustomer = await pool.query(
...
app.get("/customer", async (req, res) => {
  try {
    ...
    res.render("customer", {
      customerList: customerData.rows,
      customerSelected: false,
      customerData: [],
      error: "Please add initial values",
    });
  } catch (err) {
    console.log(err);
    res.redirect("/");
...
app.post("/customer/new", async (req, res) => {
  ...
  try {
    const customerData = await pool.query("CALL newCustomer($1,$2)", [
      formData.customerName,
      formData.customerId,
    ]);
    logData(
      "CALL newCustomer($1,$2), [formData.customerName,formData.customerId]",
      customerData.rows,
    );
...
app.post("/customer/getDetails", async (req, res) => {
  ...
  try {
    const customerData = await pool.query("SELECT getCustomerDetails($1)", [
      customer_id,
    ]);
    ...
    res.render("customer", {
      customerList: allCustomerData.rows,
...
app.post("/order/new", async (req, res) => {
  try {
    ...
    const resultData = await pool.query("CALL newOrder($1,$2,$3,$4,$5);", [
      parseInt(orderData.product_id),
...
app.post("/order/cancel", async (req, res) => {
  try {
    const orderData = req.body;
    console.log(orderData);
    const queryResult = await pool.query("call deleteOrder($1);", [
...
app.post("/feedback/new", async (req, res) => {
  try {
    const orderData = req.body;
    console.log(orderData);
    const queryResult = await pool.query("call newFeedback($1,$2,$3)", [
      String(orderData.customer_id),
      String(orderData.customer_name),
      String(orderData.feedback_desc),
    ]);
    console.log("done");
    res.redirect("/");
  } catch (err) {
